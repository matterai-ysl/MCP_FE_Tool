"""
Persistent local storage helpers for feature pipelines.
"""
...
def _ensure_pipelines_root() -> Path:
    root = _resolve_path_within_data_dir(PIPELINES_DIR)
    root.mkdir(parents=True, exist_ok=True)
    return root


def create_pipeline_dir() -> tuple[str, Path]:
    pipeline_id = str(uuid4())
    pipeline_dir = _resolve_path_within_data_dir(_ensure_pipelines_root() / pipeline_id)
...
def save_metadata(pipeline_dir: Path, metadata: Dict[str, Any]) -> Path:
...
def load_metadata(pipeline_id: str) -> Dict[str, Any]:
...
def save_transform_output(pipeline_id: str, source_name: str, csv_content: str) -> Path:
    pipeline_dir = get_pipeline_dir(pipeline_id)
